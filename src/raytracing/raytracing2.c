...
t_vector			colors(t_rtv *rtv,t_object *obj,t_vector hit, t_vector normal, t_ray ray)
{
	...
	while (tmp)
	{
		...
		alfa =  dst != 9999 ? 0.6*dot(light_dir, normal) / (length(light_dir, light_dir) * length(normal, normal)) : dot(light_dir, normal) / (length(light_dir, light_dir) * length(normal, normal));
